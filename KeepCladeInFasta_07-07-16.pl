...
# perl KeepCladeInFasta.pl refPresent clade isolatesInClades.txt sequences.fasta output.fasta 

# IsolatesInClades file format:
...
# Get the clade to keep
my $cladeToKeep = shift @ARGV;
...
# Initialise a hashtable to store the isolate IDs to be kept
my %isolatesToKeep= ();
...
while(<CLADES>){
	...
	# Store the IDs of the isolates in the clade to be kept
	$isolatesToKeep{$cols[0]} = 1 if $cols[1] == $cladeToKeep;
}
close(CLADES);
...
my $nIsolates = keys %isolatesToKeep;
my $seqLength;
...
while(<FASTA>){
	...
	if($line =~ /^>/){
		...
		if($lineNo > 2){
			...
			# Print the isolate sequence out to file if it is one of the isolates being kept
			if(exists($isolatesToKeep{$isolateId}) || $isolateId =~ /^Ref/){
...
# Print the last isolate's sequence out to file if it is one of the isolates being kept
if(exists($isolatesToKeep{$isolateId}) || $isolateId =~ /^Ref/){
	print OUTPUT ">$isolateId\n$sequence\n";
}
